...
import { ArrowDownLeft, ArrowUpRight, CalendarDays, Check, ChevronLeft, ChevronRight, CircleDollarSign, Clock3, FileChartColumn, LayoutDashboard, LogOut, Menu, Pencil, Plus, Repeat2, Search, Tags, Trash2, X } from "lucide-react";

import { CategoriesView } from "@/components/categories/categories-view";
import { EntryForm } from "@/components/entry-form";
import { RecurrencesView } from "@/components/recurrences/recurrences-view";
import { ReportsView } from "@/components/reports/reports-view";
import { createCategory, createEntry, deleteCategory, deleteEntry, getCategories, getDashboard, updateEntry } from "@/lib/api";
import { currentMonth, formatCurrency, formatDate, formatMonth, shiftMonth } from "@/lib/format";
import { createClient } from "@/lib/supabase/client";
import type { AppView, Category, DashboardData, Entry, EntryKind, EntryPayload } from "@/lib/types";
...
export function Dashboard({ user }: { user: User }) {
  ...
  async function saveEntry(payload: EntryPayload) {
    setSaving(true);
    try { if (editing) await updateEntry(editing.id, payload); else await createEntry(payload); setFormOpen(false); setEditing(null); await reloadDashboard(); } catch (caught) { setError(errorMessage(caught)); } finally { setSaving(false); }
  }

  async function removeEntry(entry: Entry) { if (!window.confirm(`Remover "${entry.description}"?`)) return; try { await deleteEntry(entry.id); await reloadDashboard(); } catch (caught) { setError(errorMessage(caught)); } }
  ...
  const entries = useMemo(() => (data?.entries ?? []).filter((entry) => (filter === "all" || entry.kind === filter) && (!search.trim() || `${entry.description} ${entry.category}`.toLocaleLowerCase("pt-BR").includes(search.trim().toLocaleLowerCase("pt-BR")))), [data?.entries, filter, search]);
...
function Overview({ month, data, entries, categories, loading, search, filter, onSearch, onFilter, onMonth, onAdd, onEdit, onDelete, onToggle }: { month: string; data: DashboardData | null; entries: Entry[]; categories: Category[]; loading: boolean; search: string; filter: Filter; onSearch: (value: string) => void; onFilter: (value: Filter) => void; onMonth: (value: string) => void; onAdd: (kind: EntryKind) => void; onEdit: (entry: Entry) => void; onDelete: (entry: Entry) => void; onToggle: (entry: Entry) => void }) {
  ...
  return <><div className="mb-5 flex items-center justify-between gap-3 rounded-2xl border border-[#dedfd8] bg-[#fbfbf8]/80 p-2 panel-shadow sm:w-fit"><button onClick={() => onMonth(shiftMonth(month, -1))} className="rounded-xl p-2 hover:bg-[#e9eae4]"><ChevronLeft size={18} /></button><label className="relative flex min-w-[175px] items-center justify-center gap-2 px-1 text-sm font-bold"><CalendarDays size={17} /><span>{formatMonth(month)}</span><input type="month" value={month} onChange={(event) => event.target.value && onMonth(event.target.value)} className="absolute inset-0 cursor-pointer opacity-0" /></label><button onClick={() => onMonth(shiftMonth(month, 1))} className="rounded-xl p-2 hover:bg-[#e9eae4]"><ChevronRight size={18} /></button></div>
    ...
    <section className="grid gap-5 xl:grid-cols-[minmax(0,1.65fr)_minmax(300px,0.75fr)]"><div className="overflow-hidden rounded-[24px] border border-[#dedfd8] bg-[#fbfbf8] panel-shadow"><div className="border-b border-[#e3e4dd] p-5 sm:p-6"><div className="mb-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><h2 className="text-lg font-semibold">Lançamentos</h2><p className="mt-1 text-sm text-[#7b827d]">Tudo que entra e sai neste mês</p></div><div className="relative sm:w-64"><Search size={17} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#8a918c]" /><input value={search} onChange={(event) => onSearch(event.target.value)} placeholder="Buscar lançamento" className="w-full rounded-xl border border-[#dcded6] bg-white py-2.5 pl-10 pr-3 text-sm outline-none" /></div></div><div className="flex gap-2">{(["all", "expense", "income"] as Filter[]).map((item) => <button key={item} onClick={() => onFilter(item)} className={`rounded-full px-4 py-2 text-xs font-bold ${filter === item ? "bg-[#17211c] text-white" : "border border-[#dcded6] bg-white text-[#68706b]"}`}>{item === "all" ? "Todos" : item === "expense" ? "Despesas" : "Receitas"}</button>)}</div></div><div className="min-h-[360px]">{loading ? <LoadingRows /> : entries.length ? entries.map((entry) => <EntryRow key={entry.id} entry={entry} onEdit={() => onEdit(entry)} onDelete={() => void onDelete(entry)} onToggle={() => void onToggle(entry)} />) : <EmptyState onAdd={() => onAdd("expense")} />}</div></div>
      <div className="space-y-5"><div className="rounded-[24px] border border-[#dedfd8] bg-[#fbfbf8] p-5 panel-shadow sm:p-6"><div className="mb-6 flex justify-between"><div><h2 className="text-lg font-semibold">Por categoria</h2><p className="mt-1 text-sm text-[#7b827d]">Onde seu dinheiro vai</p></div><Tags size={20} /></div><div className="space-y-5">{data?.categories.slice(0, 7).map((item) => { const color = categories.find((category) => category.kind === "expense" && category.name === item.category)?.color ?? "#d8ff65"; return <div key={item.category}><div className="mb-2 flex justify-between gap-3 text-sm"><span className="flex items-center gap-2"><i className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: color }} />{item.category}</span><strong>{formatCurrency(item.amount_cents)}</strong></div><div className="h-1.5 rounded-full bg-[#e8e9e3]"><div className="h-full rounded-full" style={{ width: `${Math.max(item.amount_cents / maxCategory * 100, 4)}%`, backgroundColor: color }} /></div></div>})}</div></div><div className="rounded-[24px] bg-[#d8ff65] p-6"><div className="mb-5 flex h-10 w-10 items-center justify-center rounded-full bg-[#17211c] text-[#d8ff65]"><Check size={19} /></div><p className="text-xs font-bold uppercase tracking-[0.15em] text-[#526228]">Progresso do mês</p><h3 className="mt-2 text-xl font-semibold">{data?.expense_cents ? Math.round(data.paid_expense_cents / data.expense_cents * 100) : 0}% das despesas pagas</h3><div className="mt-5 h-2 rounded-full bg-[#17211c]/15"><div className="h-full rounded-full bg-[#17211c]" style={{ width: `${data?.expense_cents ? data.paid_expense_cents / data.expense_cents * 100 : 0}%` }} /></div></div></div></section></>;
}
...
function EntryRow({ entry, onEdit, onDelete, onToggle }: { entry: Entry; onEdit: () => void; onDelete: () => void; onToggle: () => void }) { const complete = entry.status === "paid" || entry.status === "received"; return <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 border-b border-[#e7e8e2] px-4 py-4 last:border-0 sm:grid-cols-[auto_minmax(0,1fr)_115px_auto] sm:px-6"><button onClick={onToggle} className={`flex h-10 w-10 items-center justify-center rounded-xl ${entry.kind === "income" ? "bg-[#e4f4ec] text-[#287055]" : "bg-[#f8e9e6] text-[#a94c40]"}`}>{entry.kind === "income" ? <ArrowDownLeft size={18} /> : <ArrowUpRight size={18} />}</button><div className="min-w-0"><p className="truncate text-sm font-semibold">{entry.description}</p><p className="mt-1 truncate text-xs text-[#858c87]">{entry.category} · {formatDate(entry.due_date)} · {complete ? "Concluído" : "Pendente"}</p></div><p className={`text-right text-sm font-bold ${entry.kind === "income" ? "text-[#287055]" : ""}`}>{entry.kind === "income" ? "+" : "-"} {formatCurrency(entry.amount_cents)}</p><div className="col-start-2 flex justify-end gap-1 sm:col-auto"><button onClick={onEdit} className="rounded-lg p-2 text-[#7c847f] hover:bg-[#eceee8]"><Pencil size={15} /></button><button onClick={onDelete} className="rounded-lg p-2 text-[#9b7c77] hover:bg-[#fff0ed]"><Trash2 size={15} /></button></div></div>; }
